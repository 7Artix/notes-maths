...
> 关于一些变量的二次齐次多项式称为<font color=red><b>二次型</b></color> ( $Quadratic \ form$ ).
...
> 其中 $\boldsymbol{A}$ 是 $n\times n$ 的对称阵, 该矩阵 $\boldsymbol{A}$ 称为 $f$ 的<font color=red><b>二次型矩阵</b></color> ( $matrix\ of\ the\ quadratic\ form$ ).
...
先将二次型<font color=red><b>平分交叉项</b></color>, 将原二次型改写成如下形式:
...
<font color=red><b>二次型矩阵都是对称阵</b></color>.
...
> 设 $\boldsymbol{A}$ 和 $\boldsymbol{B}$ 是 $n$ 阶方阵, 若有可逆矩阵 $\boldsymbol{P}$ , 使 $\boldsymbol{B} = \boldsymbol{P}^T\boldsymbol{A}\boldsymbol{P}$ , 则称 $\boldsymbol{A}$ 和 $\boldsymbol{B}$ <font color=red><b>合同</b></color> ( $Congruence$ ) , 称 $\boldsymbol{B}$ 是 $\boldsymbol{A}$ 的<font color=red><b>合同矩阵</b></color> ( $Matrix \ Congruence$ ).
...
<font color=green><b>相似变换</b></color>的形式为 $\boldsymbol{B} = \boldsymbol{P}^{-1}\boldsymbol{A}\boldsymbol{P}$ 

> 相似变换的对象是一个<font color=red><b>线性算子</b></color>的矩阵表示, 其解决的问题为: 
> "一个 `将向量v变换到向量w` 的线性变换, 在更换了一组坐标基之后, 这个变换的矩阵应该是什么?" 
> 这个过程中只涉及一个方向向量, 即 $\boldsymbol{w} = \boldsymbol{A}\boldsymbol{v}$ .


<font color=green><b>合同变换</b></color>的形式为 $\boldsymbol{B} = \boldsymbol{P}^{T}\boldsymbol{A}\boldsymbol{P}$ 

> 合同变换的对象是一个<font color=red><b>二次型</b></color>的矩阵表示, 其解决的问题为: 
...
本质上二元二次方程的图像只有两种, 一种是<font color=red><b>封闭曲线</b></color>, 即椭圆; 另一种是<font color=red><b>开放曲线</b></color>, 即双曲线.
...
> 只含有平方项的二次型 $k_1x_1^2+k_2x_2^2+\cdots+k_nx_n^2$ 称为<font color=red><b>二次型的标准形</b></color> ( $Canonical \ form \ of \ a \ Quadratic$ ), 简称为标准形. 齐次二次型矩阵一定为 $n$ 阶对角阵:
...
合同对角化可以使用正交合同对角化, 也可以使用<font color=red><b>拉格朗日配方法</b></color>:
...
> 正系数的个数 (也称为<font color=red><b>正惯性指数</b></color>, $Positive \ index \ of \ inertia$ ), 
> 负系数的个数 (也称为<font color=red><b>负惯性指数</b></color>, $Negative \ index \ of \ inertia$ ), 
> 以及零系数的个数均相同.
> 该定理称为<font color=red><b>西尔维斯特惯性定理</b></color> ( $Sylvester's \ law \ of \ inertia$ ), 简称为<font color=red><b>惯性定理</b></color>.
...
- <font color=red><b>正定</b></color> ( $Positive \ definite$ ) 的, 若对所有的 $\boldsymbol{x}\neq\boldsymbol{0}$ , 有 $f(\boldsymbol{x})>0$ .
- <font color=red><b>半正定</b></color> ( $Positive \ semidefinite$ ) 的, 若始终有 $f(\boldsymbol{x})\geq0$ .
- <font color=red><b>负定</b></color> ( $Negative \ definite$ ) 的, 若对所有的 $\boldsymbol{x}\neq\boldsymbol{0}$ , 有 $f(\boldsymbol{x})<0$ .
- <font color=red><b>半负定</b></color> ( $Negative \ semidefinite$ ) 的, 若始终有 $f(\boldsymbol{x})\leq0$ .
- <font color=red><b>不定</b></color> ( $Indefinite$ ) 的, 若 $f(\boldsymbol{x})$ 既有正值又有负值.


## `定理4` 维尔茨定理

> <font color=red><b>维尔茨定理</b></color> ( $Hurwitz \ theorem$ ):
