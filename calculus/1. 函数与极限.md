...
> 若按照某一规则, 对每个 $n \in{\mathbb{Z^+}}$ , 对应一个确定的实数 $a_n$ , 这些实数 $a_n$ 按照下标 $n$ 从小到大排列得到一个序列, 这个序列成为<font color=red><b>数列</b></color>, 简记为 $\{a_n\}$ :

$$
\{a_n\}=\{a_1,a_2,a_3,\cdots,a_n,\cdots\}
$$

> 其中的每一个数叫做数列的**项**, 第 $n$ 项称为数列的<font color=red><b>通项</b></color> $a_n$ .
...
> 若某数列无线趋于某实数, 与该实数的差可以<font color=red><b>任意小</b></color>, 则该确定的实数称为此数列的<font color=red><b>极限</b></color>.
...
> 则称 $L$ 是数列 $\{a_n\}$ 的<font color=red><b>极限</b></color>, 或者称数列 $\{a_n\}$ <font color=red><b>收敛于</b></color> $L$ , 记作:

$$
\lim\limits_{n\to\infty}a_n=L\quad 或 \quad a_n\to L \ (n \to \infty)
$$

> 若不存在这样的常数 $L$ , 则称数列 $\{a_n\}$ 没有极限, 或称数列 $\{a_n\}$ 是<font color=red><b>发散</b></color>的, 也可以说 $\lim\limits_{n \to \infty}a_n$ 不存在.
...
> 那么称 $L$ 是数列 $\{a_n\}$ 的<font color=red><b>极限</b></color>, 或者称数列 $\{a_n\}$ <font color=red><b>收敛于</b></color> $L$ , 记作:
...
> 对于数列 $\{a_n\}$ 
- 若 $\forall n \in \mathbb{Z^+}$ , 有 $a_n \leq A$ , 则称该数列有<font color=red><b>上界</b></color> ( $Bounded \ above$ ), 且称 $A$ 为数列的上界.
- 若 $\forall n \in \mathbb{Z^+}$ , 有 $a_n \geq B$ , 则称该数列有<font color=red><b>下界</b></color> ( $Bounded \ below$  ), 且称 $B$ 为数列的下界.
- 若 $\forall n \in \mathbb{Z^+}$ , 有 $|a_n| \leq M$ , 则称该数列有<font color=red><b>界</b></color> ( $Bounded$  ), 且称 $M$ 为数列的界.
...
> 在原数列 $\{a_n\}$ 中任意抽取**无限多**项, 并且保持这些项在原数列中的**顺序**, 这样得到的数列成为原数列 $\{a_n\}$ 的**子数列**, 简称<font color=red><b>子列</b></color>.
...
以 $x_0$ 为中心, 半径为 $\delta \ (\delta > 0)$ 的开区间 $(x_0 - \delta , x_0 + \delta)$ 称为点 $x_0$ 的<font color=red><b>邻域</b></color>, 记作 $U (x_0,\delta)$ , 若不关心半径 $\delta$ , 可以简记为 $U (x_0)$ .


## `定义11` $去心邻域$

在邻域 $U (x_0,\delta)$ 中去掉中心 $x_0$ 后所得的区间  $(x_0 - \delta , x_0) \cup (x_0 , x_0 + \delta)$ 称为点 $x_0$ 的<font color=red><b>去心邻域</b></color>, 记作 $\mathring{U} (x_0, \delta)$ . 若不关心半径 $\delta$ , 可以简记为 $\mathring{U} (x_0)$ .
...
> 对于数列 $\{a_n\}$ , 若 $\lim \limits_{n \to \infty}a_n = 0$ , 则称数列 $\{a_n\}$ 为 $n \to \infty$ 时的<font color=red><b>无穷小</b></color>.
> 对于函数 $f(x)$ , 若 $\lim f(x) = 0$ , 则称函数 $f(x)$ 为此自变量变化过程 (指 $x \to x_0$ 或 $x \to \infty$ 等 ) 的<font color=red><b>无穷小</b></color>.
...
> 设函数 $f(x)$ 在 $\mathring{U} (x_0)$ 有定义. 若 $\forall M > 0$, $\exists \delta > 0$ , $\forall x \in \mathring{U} (x_0, \delta)$ , 有 $f(x) > M$ , 那么称函数 $f(x)$ 是 $x \to x_0$ 时的<font color=red><b>正无穷大</b></color>. 记作 $\lim \limits_{x \to x_0} f(x) = +\infty$ .


## `定义17` $负无穷大$

> 设函数 $f(x)$ 在 $\mathring{U} (x_0)$ 有定义. 若 $\forall M > 0$, $\exists \delta > 0$ , $\forall x \in \mathring{U} (x_0, \delta)$ , 有 $f(x) < -M$ , 那么称函数 $f(x)$ 是 $x \to x_0$ 时的<font color=red><b>负无穷大</b></color>. 记作 $\lim \limits_{x \to x_0} f(x) = -\infty$ .


## `定义16` $无穷大$

> 设函数 $f(x)$ 在 $\mathring{U} (x_0)$ 有定义. 若 $\forall M > 0$, $\exists \delta > 0$ , $\forall x \in \mathring{U} (x_0, \delta)$ , 有 $\left| f(x) \right| > M$ , 那么称函数 $f(x)$ 是 $x \to x_0$ 时的<font color=red><b>无穷大</b></color>. 记作 $\lim \limits_{x \to x_0} f(x) = \infty$ .
...
**仅可用于<font color=red><b>有限个</b></color>表达式运算**
...
> 若 $\lim \limits_{x \to -\infty} = L$ 或 $\lim \limits_{x \to +\infty} = L$ , 则直线 $y=L$ 称为函数 $f(x)$ 的<font color=red><b>水平渐近线</b></color>.
...
> 若 $\lim \limits_{x \to x_0^-} = \pm \infty$ 或 $\lim \limits_{x \to x_0^+} = \pm \infty$ , 则直线 $x = x_0$ 称为函数 $f(x)$ 的<font color=red><b>竖直渐近线</b></color>.


## `定义21` $斜渐近线$

> 若 $\lim \limits_{x \to -\infty} [f(x) - (ax+b)]= 0$ 或 $\lim \limits_{x \to +\infty} [f(x) - (ax+b)]= 0$ , 则直线 $y = ax+b$ 称为函数 $f(x)$ 的<font color=red><b>斜渐近线</b></color>.
...
<font color=red><b>单调有界必收敛</b></color>.
...
> 设函数 $f(x)$ 在 $x_0$ 点的某一邻域内有定义, 若 $\lim \limits_{x \to x_0} f(x) = f(x_0)$ 则称函数 $f(x)$ 在 $x_0$ 点<font color=red><b>连续</b></color>.
